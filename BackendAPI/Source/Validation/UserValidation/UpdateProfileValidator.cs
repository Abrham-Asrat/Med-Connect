...
namespace BackendAPI.Source.Validation.UserValidation
{
    public class UpdateProfileValidator : AbstractValidator<UpdateProfileDto>
    {
        ApplicationDbContext appContext;

        public UpdateProfileValidator(ApplicationDbContext _appContext)
        {
            ...
            RuleFor(u => u).Custom(
                (u, context) =>
                {
                    ...
                    {
                        context.AddFailure("Payload", @"At least one field is required to update a user profile. Valid fields are FirstName, LastName, Email, ProfilePicture, Phone, Gender, DateOfBirth, Address, MedicalHistory, EmergencyContactName, EmergencyContactPhone, Specialties, Availabilities, Qualifications, Biography and DoctorStatus");
                    }
                }
                );

            RuleFor(u => u.UserId).NotEmpty().WithMessage("User Id must be provided to edit a profile").Must(uid => Guid.TryParse(uid, out _)).WithMessage("Not valid Guid");
        }

        public override async Task<ValidationResult> ValidateAsync(
   ValidationContext<UpdateProfileDto> context,
   CancellationToken cancellation = default
 )
        {
            UpdateProfileDto v = context.InstanceToValidate;
            ...
            var user = appContext.Users.Where(us => us.UserId == v.UserId.ConvertToGuid()).FirstOrDefault();
            if (user == null)
            {
            ...
            {
                When(
                  u => u.Specialties != null,
                  () =>
                  {
                      RuleForEach(u => u.Specialties)
            ...
            .Must(avail => Enum.TryParse<DayOfWeek>(avail.AvailableDay.ToString(), true, out _))
            .WithMessage(
            ...
            .Must(education => ValidationHelper.BeAValidDateOnlyString(education.GraduationDate))
            .WithMessage("Graduation Date should be a valid date! (yyyy-MM-dd)")
            .Must(education =>
              !string.IsNullOrEmpty(education.GraduationDate)      
              && DateOnly.TryParse(education.GraduationDate, out var sd)
            )
