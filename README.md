...
```mermaid
graph TD
    A[Angular Patient App] -->|HTTPS/JWT| B[ .NET 8 Web API ]
    C[Angular Doctor App] -->|HTTPS/JWT| B
    D[Angular Admin App] -->|HTTPS/JWT| B
    B --> E[(PostgreSQL/SQL Server)]
    B --> F[SignalR Hub]
    B --> G[MailKit / SMTP]
    B --> H[Chapa Payment Gateway]
```
